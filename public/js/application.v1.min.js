...
hermes.controller("single_equipos_controller",function($scope,$stateParams,$location,$window,Progres,Query,materialize,config){Query.getUrl(config.databaseURL+config.equipos+"/"+$stateParams.id);Query.getAll(function(equipo){$scope.equipo=equipo.data;console.log(equipo.data);Progres.loaded()});$scope.update=function(datos){Progres.progressloading();Query.updateDates(datos)};$scope.delete=function(){Progres.progressloading();Query.killme()};$scope.verModulos=function(){Materialize.toast("Esta funcion aun no esta lista",4e3)}});
...
hermes.controller("single_switches_controller",function($scope,$http,$stateParams,$location,$window,Progres,Query,materialize,config){Query.getUrl(config.databaseURL+config.switches+"/"+$stateParams.id);Query.getAll(function(sw){$scope.switche=sw.data;Progres.loaded();Query.getUrl(config.databaseURL+config.switches+"/"+$stateParams.id+config.puertos);Query.getAll(function(puertos){for(var i=0;i<puertos.data.length;i++){Query.getUrl(config.databaseURL+config.puertos+"/"+puertos.data[i].id+config.equipos);Query.getAll(function(equipos){$("#puerto").append("<p>"+equipos.data.name+"</p>");for(var i=0;i<equipos.data.equipo.length;i++){$("#equipo").append("<p>"+equipos.data.equipo[i].name+"</p>")}})}})});$scope.update=function(datos){Progres.progressloading();Query.updateDates(datos)};$scope.delete=function(){Progres.progressloading();Query.killme()};$scope.verModulos=function(){Materialize.toast("Esta funcion aun no esta lista",4e3)}});

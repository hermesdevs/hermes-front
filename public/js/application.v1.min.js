var hermes=angular.module("hermes",["ui.router","ui.materialize"]);hermes.constant("config",{apiKey:"AIzaSyB5iw0dc-qipE6iIEeoezD2oetkHh0s8ls",databaseURL:"http://hermes.api.dev",usuarios:"/usuarios",equipos:"/equipos",servidores:"/servidores",switches:"/switches",puertos:"/puertos",vlans:"/vlans",version:"0.1.2"});hermes.constant("Switches",{general:"http://hermes.api.dev/switches",single:"http://hermes.api.dev/switches/"});
...
hermes.controller("dashboard_controller",function($scope,$location,$window,Progres,Query,materialize,config,Session,Auth){Auth.getUser(function(user){$scope.user=user.data;$scope.user.active=true});Query.getUrl(config.databaseURL+config.switches);Query.getAll(function(switches){$scope.switches=switches.data;Progres.loaded()});Query.getUrl(config.databaseURL+config.servidores);Query.getAll(function(servidores){$scope.servidores=servidores.data;Progres.loaded()});Query.getUrl(config.databaseURL+config.equipos);Query.getAll(function(equipos){$scope.equipos=equipos.data;Progres.loaded()});Query.getUrl(config.databaseURL+config.puertos);Query.getAll(function(puertos){$scope.puertos=puertos.data;Progres.loaded()});$scope.verModulos=function(){Materialize.toast("Esta funcion aun no esta lista",4e3)}});
hermes.controller("equipos_controller",function($scope,Query,Equipos,Progres){Query.getAll(Equipos.general,function(equipos){$scope.equipos=equipos.data;Progres.loaded()})});
...
hermes.controller("profile_controller",function($scope,$location,$window,Progres,Session,Auth,Query,config){Auth.getUser(function(user){$scope.user=user.data;$scope.user.active=true});$scope.updateInfoUser=function(datos){Auth.updateUser(datos)}});
...
hermes.controller("single_equipos_controller",function($scope,Query,Equipos,$stateParams,Progres){Query.getAll(Equipos.single,function(equipo){$scope.equipo=equipo.data;Progres.loaded()});$scope.update=function(datos){Progres.progressloading();Query.updateDates(Equipos.single,datos)};$scope.delete=function(){Progres.progressloading();Query.killme(Equipos.single)};$scope.verModulos=function(){Materialize.toast("Esta funcion aun no esta lista",4e3)}});
...
hermes.service("Equipos",function(config,$stateParams){return{general:config.databaseURL+config.equipos,single:config.databaseURL+config.equipos+"/"+$stateParams.id,puertos:config.databaseURL+config.equipos+"/"+$stateParams.id+config.puertos,puertosEquipos:config.databaseURL+config.equipos+"/"+$stateParams.id+config.equipos,puertosSwitches:config.databaseURL+config.equipos+"/"+$stateParams.id+config.equipos,puertosServidores:config.databaseURL+config.equipos+"/"+$stateParams.id+config.servidores}});

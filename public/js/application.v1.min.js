...
hermes.controller("single_switches_controller",function($scope,$resource,$http,$stateParams,$location,$window,Progres,Query,materialize,config){Query.getUrl(config.databaseURL+config.switches+"/"+$stateParams.id);Query.getSingle(function(sw){$scope.switche=sw.data});$scope.update=function(datos){Query.updateDates(datos)};$scope.delete=function(){Query.killme()}});
...
hermes.service("Progres",function(){this.loading=function(argument){};this.loaded=function(argument){$(".progress").addClass("hide");$(".load").addClass("hide");$(".table__content").removeClass("hide")};this.itsClear=function(argument){$(".loadClear").removeClass("hide")};this.fuckMenSalioMal=function(argument){$(".loadError").removeClass("hide")}});
hermes.service("Query",function($http,$window,config){this.getUrl=function(url){this.url=url};this.getAll=function(success,failure){$http.get(this.url).success(success).error(failure)};this.getSwitcheEquipo=function(success,failure){$http.get(this.url).success(success).error(failure)};this.getSingle=function(success,failure){$http.get(this.url).success(success).error(failure)};this.sendNudes=function(datos,success,failure){$http({url:config.databaseURL+config.switches,method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},data:$.param(datos)}).then(function(response){Materialize.toast("Tu switche fue creado",4e3);console.log(JSON.stringify(response))},function(response){Materialize.toast("Ocurrio un error",4e3);console.log(JSON.stringify(response))})};this.updateDates=function(datos){$http({url:this.url,method:"PUT",headers:{"Content-Type":"application/x-www-form-urlencoded"},data:$.param(datos)}).then(function(response){Materialize.toast("Tu switche fue actualizado",4e3);console.log(JSON.stringify(response))},function(response){Materialize.toast("Ocurrio un error",4e3);console.log(JSON.stringify(response))})};this.killme=function(){$http({url:this.url,method:"DELETE",headers:{"Content-Type":"application/x-www-form-urlencoded"}}).then(function(response){Materialize.toast("Tu switche fue eliminado",4e3);$window.location.href="#/switches";console.log(JSON.stringify(response))},function(response){Materialize.toast("Ocurrio un error al eliminar tu switche",4e3);console.log(JSON.stringify(response))})}});
